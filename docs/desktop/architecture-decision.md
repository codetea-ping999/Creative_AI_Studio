...
The initial desktop shell will connect to an already-running backend. A later phase may package the backend as a Tauri sidecar behind a `BackendSupervisor` abstraction.
...
This separation allows the companion to remain available while the heavier Studio WebView is hidden or destroyed.
...
The full Studio WebView is not required to remain resident indefinitely. A later power-management phase may destroy an inactive Studio window and recreate it on demand.
...
Browser runtime resolves the configured web API URL. Desktop runtime may later resolve a dynamically supervised local backend endpoint through Tauri IPC.
...
- no Desktop-spawned Python process;
...
2. Runtime adapter
   - browser/desktop API endpoint abstraction.
...
4. Power management
   ...
   - optional Studio WebView destruction after inactivity.

5. Backend supervisor
   - external backend first;
   - packaged sidecar later;
   - health checks, dynamic endpoint, clean shutdown.
...
- Tauri/WebView differences must be tested across Windows and macOS.
- Transparent/click-through behavior may require platform-specific fallback code.
- Desktop runtime endpoint discovery requires a small web-platform abstraction.
- Sidecar packaging and process cleanup remain a separate engineering problem and must not leak into Desktop Shell v0.1.
