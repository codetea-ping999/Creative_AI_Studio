...
class RuntimeEntry:
    ...
    def __init__(
        ...
        *,
        ...
        #
        # PR4a v1 final-convergence pass: this used to be paired with an
        # `execution_lock_owner` field so `ModelService._acquire_execution_lock()`
        # could fail fast on a same-thread recursive acquisition of this
        # exact entry (reachable with `admission_capacity > 1`, where the
        # process-wide admission gate alone would not have caught it). That
        # field was removed once `RuntimeAdmissionController.acquire()` was
        # changed to unconditionally reject *any* nested `acquire_runtime()`
        # call from a thread that already holds an open `RuntimeHandle`,
        # regardless of canonical id or admission capacity -- see that
        # class's own docstring in `core/models/service.py`. A same-thread
        # recursive acquisition of this exact entry is now rejected at that
        # earlier point and can never reach this lock at all, so
        # `execution_lock` is used exactly like an ordinary, non-reentrant
        # `threading.Lock`, contended only across genuinely different
        # threads.
