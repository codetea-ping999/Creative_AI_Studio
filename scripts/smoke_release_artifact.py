...
def verify_ui(client: httpx.Client, artifact_root: Path) -> None:
...
def run_stable_journey(client: httpx.Client, artifact_root: Path) -> tuple[str, str]:
    ...
    # 1. Project
    r = client.post(f"{BASE_URL}/projects", json={"name": "Stable film"})
    check("POST /projects 201", r.status_code == 201)
    ...
    check("POST /stories 201", r.status_code == 201)
    story_id = r.json()["id"]

    # 3. Expand (template-writer)
    r = client.post(
        f"{BASE_URL}/stories/{story_id}/expand",
        json={"task": "scene_list", "model_id": "template-writer", "params": {"scene_count": SCENE_COUNT}},
    )
    check("POST /stories/{id}/expand 201", r.status_code == 201)
    expand_job_id = r.json()["job_id"]
    j = poll_job(client, expand_job_id)
    check("expand job succeeded", j.get("status") == "succeeded")

    # 4. Apply
    r = client.post(f"{BASE_URL}/stories/{story_id}/apply", json={"job_id": expand_job_id})
    check("POST /stories/{id}/apply 200", r.status_code == 200)
    scenes = r.json().get("story", {}).get("scenes", [])
    ...
    for sid in scene_ids:
        ...
        check(f"POST /scenes/{sid}/generate 201", r.status_code == 201)
        jid = r.json()["job_id"]
        visual_jobs[sid] = jid
    ...
    detail = client.get(f"{BASE_URL}/stories/{story_id}").json()
    ...
    gallery = client.get(f"{BASE_URL}/gallery", params={"project_id": project_id, "media_type": "video"}).json()
    ...
    check("POST /stories/{id}/assemble 201", r.status_code == 201)
    asm_job_id = r.json()["job_id"]
    j = poll_job(client, asm_job_id)
    check("assembly job succeeded", j.get("status") == "succeeded")

    # 9. Gallery shows exactly one MP4
    gallery2 = client.get(f"{BASE_URL}/gallery", params={"project_id": project_id, "media_type": "video"}).json()
